    ...
    suspend fun complete(settings: AppSettings, profile: ModelProfile, messages: List<ChatMessage>, tools: AgentToolExecutor? = null, onProgress: (AgentProgress) -> Unit = {}): AiResult {
        ...
        val systemPrompt = buildString {
            ...
            if (tools != null) {
                ...
                if (settings.termuxExecutionEnabled) append(" Для запуска Node.js, npm, git, компиляторов и тестов используй run_command: команда выполняется через Termux в папке текущего Android-проекта, не на ПК.")
